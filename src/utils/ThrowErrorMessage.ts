export function throwErrorMessage(message: string, element: string) {
  
  const errorMessageContainer = document.createElement('div');
  const infoIcon = `
  <svg xmlns="http://www.w3.org/2000/svg"
      width="16"
      height="16"
      fill="none"
      viewBox="0 0 16 16"
      ><path
        stroke="currentColor"
        stroke-linecap="round"
        stroke-linejoin="round"
        d="M2 8a6 6 0 1 0 12 0A6 6 0 0 0 2 8Z"></path><path
        fill="currentColor"
        d="M8.004 10.462V7.596ZM8 5.57v-.042Z"></path><path
        stroke="currentColor"
        stroke-linecap="round"
        stroke-linejoin="round"
        d="M8.004 10.462V7.596M8 5.569v-.042"></path></svg>`;

  errorMessageContainer.classList.add('error');
  errorMessageContainer.insertAdjacentHTML("afterbegin", infoIcon);
  errorMessageContainer.insertAdjacentHTML("beforeend", message);

  const inputField = document.getElementById(`${element}`) as HTMLElement;
  
  // const inputField = document.querySelector(`input[name="${element}"`) as HTMLInputElement;
  inputField?.insertAdjacentElement("afterend", errorMessageContainer);

  setTimeout(() => {
    errorMessageContainer.remove();
    inputField.style.border = "initial";
  }, 5000);
}